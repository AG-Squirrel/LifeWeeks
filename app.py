...
def survival_from_birth(age_years: np.ndarray) -> np.ndarray:
    return np.exp(
        - c*age_years
        - (alpha/beta)*np.exp(-beta*anchor_age)*(np.exp(beta*age_years) - 1.0)
    )
...
st.set_page_config(page_title="Life Weeks: Risk & Survival", page_icon="📅", layout="centered")
st.title("Life Weeks")
...
show_risk = st.toggle("Show Mortality Risk", value=True)

st.caption(
    "Each dot is a week. Past weeks are black. Toggle adds a log-scale risk overlay from present date. "
    "Greyscale encodes survival probability. Black ring marks the present date."
)
...
is_past = (y < present_row) | ((y == present_row) & (x < present_col))
...
s_plot = np.where(is_past, 1.0, s_vals)
ax.scatter(
    x, y,
    c=s_plot, cmap="Greys", norm=norm_surv,
    s=dot_size, marker="o", edgecolors="none", zorder=4, alpha=1.0
)

if show_risk:
    risk_vmin, risk_vmax = 1e-4, 1e-1
    norm_risk = LogNorm(vmin=risk_vmin, vmax=risk_vmax)
    cmap_risk = plt.get_cmap("jet")
    risk_rgba = cmap_risk(norm_risk(np.clip(mu_vals, risk_vmin, risk_vmax)))
    risk_rgba[:, 3] = np.clip(fade_base + fade_gain * s_vals, 0.0, 1.0)
    risk_rgba[is_past, 3] = 0.0
    ax.scatter(x, y, s=dot_size, marker="o",
               facecolors=risk_rgba, edgecolors="none", zorder=5)
...
if show_risk:
    ...
    norm_risk_for_cbar = LogNorm(vmin=1e-4, vmax=1e-1)
    cbar_risk = fig.colorbar(
        plt.cm.ScalarMappable(cmap="jet", norm=norm_risk_for_cbar),
        cax=cax_risk, orientation="vertical"
    )
    cbar_risk.set_label("Annual Mortality Risk", fontsize=8, labelpad=-25)
    cbar_risk.ax.yaxis.set_label_position('left')
    cbar_risk.ax.yaxis.set_ticks_position('left')
    cbar_risk.set_ticks([1e-1, 1e-2, 1e-3, 1e-4])
    cbar_risk.set_ticklabels(["1/10","1/100","1/1000","1/10000"])
    cbar_risk.ax.yaxis.set_major_locator(LogLocator(base=10))
...
cbar_surv.set_label("Survival", fontsize=8, labelpad=-10)
...
suffix = "risk_on" if show_risk else "risk_off"
col_dl1, col_dl2 = st.columns(2)
with col_dl1:
    st.download_button("Download PNG (600 dpi)", data=buf_png,
                       file_name=f"life_weeks_{suffix}_{birth_date.strftime('%Y%m%d')}.png",
                       mime="image/png")
with col_dl2:
    st.download_button("Download PDF (vector)", data=buf_pdf,
                       file_name=f"life_weeks_{suffix}_{birth_date.strftime('%Y%m%d')}.pdf",
